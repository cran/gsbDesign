...
gsbSimulation <- function(truth=NULL,
                          ...
                          grid.type= c("table","plot","manually"),
                          nr.sim= 50000,
  ...
  if (type.update[1]=="treatment effect" && method == "numerical integration"){
    if (grid.type=="manually"){
      ...
      r <- list(truth=truth,type.update=type.update,method=method)
    }else{
      ...
      r <- list(truth=seq(truth[1],truth[2],length.out=truth[3]),type.update=type.update,method=method, seed=seed)
    }
...
tab <- function(OC, what = c("success", "futility", "success or futility", 
    "cumulative success", "cumulative futility", "cumulative success or futility", 
    "sample size"), atDelta = "default", digits = 4, export = FALSE, 
    sep = ",", path = getwd()) 
{
    what <- match.arg(what)
    ...
    if (class(digits) != "numeric" | length(digits) != 1 | round(digits, 
        digits = 0) != digits) 
    ...
    if (class(OC)[1] == "gsbMainOut") {
       OC <- OC$OC
    }

    
    if (class(OC)[1] == "gsbSimArm.result") {
        if (!(what == "sample size" & digits == 0)) {
            ...
            value <- array(dim = c(length(delta.control), length(delta.treatment), 
                i), data = subset(OC, OC$type == what)$value)
            value <- round(value, digits = digits)
            dimnames(value) <- list(c(paste("contr", round(delta.control, 
                digits))), c(paste("treat", round(delta.treatment, 
                digits))), paste("stage", 1:i))
            result <- value
        }
        if ((what == "sample size" & digits == 0)) {
            OC <- OC[order(OC$delta.control), ]
            ...
            value <- array(dim = c(length(delta.control), length(delta.treatment), 
                i), data = subset(OC, OC$type == what)$value)
            value <- ceiling(value)
            dimnames(value) <- list(c(paste("plac", round(delta.control, 
                digits))), c(paste("treat", round(delta.treatment, 
                digits))), paste("stage", 1:i))
            result <- value
        }
    ...
    if (class(OC)[1] != "gsbSimArm.result") {
        if (atDelta[1] == "default") {
            if (!(what == "sample size" & digits == 0)) {
                if (length(levels(OC$method)) == 2) {
                  OC <- subset(OC, OC$method == "numerical integration")
                }
                OC <- OC[order(OC$stage), ]
                OC <- OC[order(OC$type), ]
                delta <- OC$delta[!duplicated(OC$delta)]
                i <- length(levels(OC$stage))
                value <- matrix(nrow = length(delta), ncol = i, 
                  data = subset(OC, OC$type == what)$value, byrow = FALSE)
                dimnames(value) <- list(1:length(delta), paste("stage", 
                  1:i))
                result <- round(cbind(delta, value), digits = digits)
            }
            if ((what == "sample size" & digits == 0)) {
                if (length(levels(OC$method)) == 2) {
                  OC <- subset(OC, OC$method == "numerical integration")
                ...
                dimnames(value) <- list(1:length(delta), paste("stage", 
                  1:i))
                result <- ceiling(cbind(delta, value))
            }
        }
        if (atDelta[1] != "default") {
            if (sum(min(OC$delta) <= atDelta & atDelta <= max(OC$delta)) != 
                length(atDelta)) {
                stop("\"atDelta\" has to be within the range of true values (OC$delta).")
            }
            if (!(what == "sample size" & digits == 0)) {
                if (length(levels(OC$method)) == 2) {
                  OC <- subset(OC, OC$method == "numerical integration")
                ...
                dimnames(value) <- list(1:length(delta), paste("stage", 
                  1:i))
                val <- matrix(nrow = length(atDelta), ncol = i)
                for (k in 1:i) {
                  val[, k] <- approx(delta, value[, k], xout = atDelta, 
                    method = "linear")$y
                }
                dimnames(val) <- list(1:length(atDelta), paste("stage", 
                  1:i))
                result <- cbind(atDelta, round(val, digits = digits))
            }
            if ((what == "sample size" & digits == 0)) {
                if (length(levels(OC$method)) == 2) {
                  OC <- subset(OC, OC$method == "numerical integration")
                ...
                dimnames(val) <- list(1:length(atDelta), paste("stage", 
                  1:i))
                result <- cbind(atDelta, ceiling(val))
            }
        }
    }
    if (export) {
...
summary.gsbMainOut <- function(object, atDelta="default", ...){
  ...
  if (class(x$OC)[1]!="gsbSimArm.result"){
    if(class(x$OC)[1]== "gsbSimDif.result"){
    ...
    if(class(x$OC)[1]== "gsbCalcDif.result" | class(x$OC)[1]== "gsbSimCalcDif.result"){
    ...
    if(atDelta[1]=="default"){
      if(i>1){
        ...
        if(logic.suc){
          ...
          suc <- tab(OC=x$OC, what="success", atDelta=val.suc, digits=4)
          dim.suc <- dim(suc)
          total <- tab(OC=x$OC, what="cumulative success", atDelta=val.suc, digits=4)[,dim.suc[2]]
          en <- tab(OC=x$OC, what="sample size", atDelta=val.suc, digits=1)[,dim.suc[2]]
        ...
        if(logic.fut){
          ...
          fut <- tab(OC=x$OC, what="futility", atDelta=val.fut, digits=4)
          dim.fut <- dim(fut)
          total <- tab(OC=x$OC, what="cumulative futility", atDelta=val.fut, digits=4)[,dim.fut[2]]
      ...
      if(i==1){
        ...
        if(logic.suc){
          ...
          suc <- tab(OC=x$OC, what="success", atDelta=val.suc, digits=4)
          en <- tab(OC=x$OC, what="sample size", atDelta=val.suc, digits=1)[,2]
        ...
        if(logic.fut){
          ...
          table.f <- tab(OC=x$OC, what="futility", atDelta=val.fut, digits=4)
          dimnames(table.f)[[2]][1] <- "delta"
    ...
    if(atDelta[1]!="default"){
      if(i>1){
        suc <- tab(OC=x$OC, what="success", atDelta=atDelta, digits=4)
        dim.suc <- dim(suc)
        total <- tab(OC=x$OC, what="cumulative success", atDelta=atDelta, digits=4)[,dim.suc[2]]
        en <- tab(OC=x$OC, what="sample size", atDelta=atDelta, digits=1)[,dim.suc[2]]
        ...
        ## table futility
        fut <- tab(OC=x$OC, what="futility", atDelta=atDelta, digits=4)
        total <- tab(OC=x$OC, what="cumulative futility", atDelta=atDelta, digits=4)[,dim.suc[2]]
        table.f <- cbind(fut,total)
        dimnames(table.f)[[2]][1] <- "delta"
      ...
      if(i==1){
        suc <- tab(OC=x$OC, what="success", atDelta=atDelta, digits=4)
        en <- tab(OC=x$OC, what="sample size", atDelta=atDelta, digits=1)[,2]
        ...
        ## table futility  
        table.f <- tab(OC=x$OC, what="futility", atDelta=atDelta, digits=4)
        dimnames(table.f)[[2]][1] <- "delta"
  ...
  if (class(x$OC)[1]=="gsbSimArm.result"){
    ...
    print(dat,digits=3, row.names=FALSE)
    cat("\nsigma treatment:", x$design$sigma[1], "\tsigma control:", x$design$sigma[2],"\n")
    ## set length of tables
    if(i>1){    
      what <- "success"
      whatcum <- "cumulative success"
      if(sum(!is.na(subset(as.data.frame(x$design$criteria),as.data.frame(x$design$criteria)$type=="success")$value ))==0){
        what <- "futility"
        whatcum <- "cumulative futility"
      }
      n <- 3 #length of output matrix
      m <- length(tab(x$OC,what,digits=4)[,1,1])
      index <- floor(seq(1,m,length.out=n))
      if(n<m){
        index <- floor(seq(1,m,length.out=n))
      }else{
        index <- 1:m
      ...
      n2 <- 6
      m2 <- length(tab(x$OC,what,digits=4)[1,,1])
      if(n2<m2){
        index2 <- floor(seq(1,m2,length.out=n2))
      }else{
        index2 <- 1:m2
      }
      
      ## tables
      s <- tab(x$OC,what,digits=4)
      if(what=="success"){
        for(j in 1:i){
          cat("\nstopping for success in stage", j,":\n")
          print(s[index,index2,j])
        }
        cat("\ntotal probability of stopping for success:\n")
      }
      if(what!="success"){
        for(j in 1:i){
          cat("\nstopping for futility in stage", j,":\n")
          print(s[index,index2,j])
        }
        cat("\ntotal probability of stopping for futility:\n")
      }
      
      ts <- tab(x$OC,whatcum,digits=4)[index,index2,i]
      print(ts)
      
      cat("\nE{N}:\n")
      sam <- tab(x$OC,"sample size",digits=1)[index,index2,i]
      print(sam)
      
      cat("\nget other tables with \"tab()\".\n")
      
      l <- list(success=s,total.success=ts,sample.size=sam)
    }
    if(i==1){
      what <- "success"
      if(sum(!is.na(subset(as.data.frame(x$design$criteria),as.data.frame(x$design$criteria)$type=="success")$value ))==0)
      ...
      s <- tab(x$OC,what,digits=4)
      if(what=="success"){
        cat("\nstopping for success in stage 1:\n")
      }
      if(what!="success"){
        cat("\nstopping for futility in stage 1:\n")       
      }
      print(s[index,index2,1])
      cat("\nE{N}:\n")
      sam <- tab(x$OC,"sample size",digits=1)[index,index2,1]
      print(sam)
...
plot.gsbMainOut <- function(x,
                            what=c("all", "cumulative all", "both", "cumulative both","sample size", "success", "futility", "success or futility", "cumulative success", "cumulative futility", "cumulative success or futility", "boundary","std.boundary","delta.grid", "patients"),
                            range.delta="default",
                            stages="default",
                            delta.grid=TRUE,
                            color=TRUE,
  ...
  ## ## x <- arm
  ## what <- "success"
  ## range.delta.placebo="default"
  ## range.delta.treatment="default"
  ## stages="default"
  ## delta.grid=TRUE
  ## color <- TRUE
  ## smooth=100
...
plot.gsbSimDif.result <- function(x,
  ...
  if(what=="all"){
    ...
    d.s <- subset(d1,d1$type=="success")
    d.s <- data.frame(d.s, plot="success")
    d.f <- subset(d1,d1$type=="futility")
    d.f <- data.frame(d.f, plot="futility")
    d.sf <- subset(d1,d1$type=="success or futility")
    d.sf$value <- 1-d.sf$value
    d.sf <- data.frame(d.sf, plot="indeterminate")
    
    d1 <- rbind(d.s,d.sf,d.f)
    ##d1$stage <- factor(d1$stage, levels=paste("stage",length(levels(d1$stage)):1))
    r <- xyplot(value~delta|stage,
                ...
                key=list(columns=3,space="bottom",
                  ...
                  text=list(as.character(d1$plot[!duplicated(d1$plot)]),col=1), border=FALSE)
                )
  }
  ...
  if(what=="cumulative all"){
    ...
    d.s <- subset(d1,d1$type=="cumulative success")
    d.s <- data.frame(d.s, plot="success")
    d.f <- subset(d1,d1$type=="cumulative futility")
    d.f <- data.frame(d.f, plot="futility")
    d.sf <- subset(d1,d1$type=="cumulative success or futility")
    d.sf$value <- 1-d.sf$value
    d.sf <- data.frame(d.sf, plot="indeterminate")
...
## d1$value[d1$method=="simulation"] <- d1$value[d1$method=="simulation"] + .25
  if(what=="both"){
    d1 <- subset(x$OC,
                 x$OC$type%in%c("success","futility","success or futility") & as.numeric(x$OC$stage)%in%stages & x$OC$delta>= range.delta[1] & x$OC$delta<= range.delta[2])
    ...
    r <- xyplot(value~delta|stage+type,
                ...
                key=list(columns=2,space="bottom",
                            lines=list(col=c("blue1","red1"),lwd=2.5),
  ...
  if(what=="sample size"){
    ...
    if(length(stages)!=1){
      col <- 2:(i+1)
      col.leg <- (2:(i+1))[2:(i+1)%in%(as.numeric(d1$stage[!duplicated(d1$stage)])+1)]
    }
    ...
    r <- xyplot(value~delta,
                ...
                scales=list(alternating=1),
                key=list(columns=ifelse(st>4,4,st), rows= ceiling(st/4),
                  col=col.leg, space="bottom",
                  lines=list(lwd=2.5),
...
## gsbPriorPreparation <- function(design){
##   if(!is.null(design$prior.difference)){
##     if(design$prior.difference[1]=="non-informative")
##       design$prior <- c(0,0,0,1,0,1)
##     if(design$prior.difference[1]!="non-informative")
##       design$prior <- c(design$prior.difference[1],NA,design$prior.difference[2], design$sigma[1,1], design$prior.difference[2],design$sigma[1,1] )
##     design$prior[2] <- (design$prior.difference[3]^2/design$prior.difference[2]+design$prior.difference[5]^2/design$prior.difference[4])^(-1)
##   }
##   design
## }
...
gsbCalcDif <- function(design,
  ...
  if (i==1){
    value <- c(as.vector(t(test$upper$prob)),as.vector(t(test$lower$prob)),as.vector(t(test$upper$prob))+as.vector(t(test$lower$prob)), rep( EX[1,1]+EX[1,3],times=length(DELTA)))
    delta <- rep(DELTA,times=i*4)
    type <- c(rep("success",times=length(DELTA)*i),rep("futility",times=length(DELTA)*i),rep("success or futility",times=length(DELTA)*i),rep("sample size",times=length(DELTA)*i))
    stage <- rep(paste("stage",t(array(dim=c(i,length(DELTA)),data=1:i))),times=4)
    method <- rep("numerical integration",times=length(DELTA)*i*4)
  }else{
    ...
    value <- c(as.vector(t(test$upper$prob)),as.vector(t(test$lower$prob)),as.vector(t(test$upper$prob))+as.vector(t(test$lower$prob)),as.vector(t(cum$cum.suc[,])), as.vector(t(cum$cum.fut[,])), as.vector(t(cum$cum.suc.fut[,])), as.vector(t(ss)))
    
    delta <- rep(DELTA,times=i*7)
    type <- c(rep("success",times=length(DELTA)*i),rep("futility",times=length(DELTA)*i),rep("success or futility",times=length(DELTA)*i),rep("cumulative success",times=length(DELTA)*i),rep("cumulative futility",times=length(DELTA)*i),rep("cumulative success or futility",times=length(DELTA)*i),rep("sample size",times=length(DELTA)*i))
    stage <- rep(paste("stage",t(array(dim=c(i,length(DELTA)),data=1:i))),times=7)
    method <- rep("numerical integration",times=length(DELTA)*i*7)
  }
...
#                                         jn <- 1
    ...
    if (i==1){
        ...
        method <- factor(rep("simulation",times=i*10*length(delta3)))

        value <- c(Psuccess,Pfutility,Psuccess.and.futility,nr.of.success,nr.of.futility,nr.of.success.and.futility, as.vector((li$success[,])), as.vector((li$futility[,])), as.vector((li$success_and_futility[,])),as.vector(sample.size))

        type <- factor(c(rep("success given",times=(i*length(delta3)) ),rep("futility given",times=(i*length(delta3)) ),rep("success or futility given",times=(i*length(delta3)) ),rep("nr of success given",times=(i*length(delta3)) ),rep("nr of futility given",times=(i*length(delta3)) ),rep("nr of success or futility given",times=(i*length(delta3))),rep("success",times=(i*length(delta3))),rep("futility",times=(i*length(delta3))),rep("success or futility",times=(i*length(delta3))),rep("sample size",times=length(delta3)) ))

        stage <- c(rep(paste("stage",1:i),times=length(delta3)*10 ))

        delta <- c(as.vector( t(array(dim=c(length(delta3)*10,i),data=delta3))))
    ...
    if (i>1){
        ...
        method <- factor(rep("simulation",times=i*13*length(delta3)))

        value <- c(Psuccess,Pfutility,Psuccess.and.futility,nr.of.success,nr.of.futility,nr.of.success.and.futility, as.vector((li$success[,])), as.vector((li$futility[,])), as.vector((li$success_and_futility[,])),as.vector((cum$cum.suc)),as.vector((cum$cum.fut)),as.vector((cum$cum.suc.fut)),as.vector(sample.size)  )

        type <- factor(c(rep("success given",times=(i*length(delta3)) ),rep("futility given",times=(i*length(delta3)) ),rep("success or futility given",times=(i*length(delta3)) ),rep("nr of success given",times=(i*length(delta3)) ),rep("nr of futility given",times=(i*length(delta3)) ),rep("nr of success or futility given",times=(i*length(delta3))),rep("success",times=(i*length(delta3))),rep("futility",times=(i*length(delta3))),rep("success or futility",times=(i*length(delta3))),rep("cumulative success",times=(i*length(delta3))),rep("cumulative futility",times=(i*length(delta3))),rep("cumulative success or futility",times=(i*length(delta3))), rep("sample size",times=(i*length(delta3))) ))
        stage <- c(rep(paste("stage",1:i),times=length(delta3)*13 ) )

        delta <- c(as.vector( t(array(dim=c(length(delta3)*13,i),data=delta3))))
...
gsbSimArm <- function(design,
 ...
 #test if patients =0 in stages
    ...
    if (i==1){
        value <- c(as.vector(t(Psuc)),as.vector(t(Pfut)),as.vector(t(Psucfut)), rep( EX[1,1]+EX[1,3],times=length(delta.grid[,1])))

        type <- c(rep("success",times=ld.p*i),rep("futility",times=ld.p*i),rep("success or futility",times=ld.p*i),rep("sample size",times=ld.p*i))

        delta.control <- rep(delta.p,times=i*4)
        delta.treatment <- rep(delta.t,times=i*4)
        delta <- delta.treatment-delta.control

        stage <- as.vector(t(array(data=paste("stage",1:i),dim=c(i,ld.t))))

        method <-  as.vector(rep("simArm",times=4*ld.t))

        r <- data.frame(value,type,delta.control,delta.treatment,stage,method)
    }else{
        ...
        value <- c(as.vector(t(Psuc)),as.vector(t(Pfut)),as.vector(t(Psucfut)), as.vector(t(cum$cum.suc)),as.vector(t(cum$cum.fut)), as.vector(t(cum$cum.suc.fut)), as.vector(t(ss)))

        type <- c(rep("success",times=ld.p*i),rep("futility",times=ld.p*i),rep("success or futility",times=ld.p*i),rep("cumulative success",times=ld.p*i),rep("cumulative futility",times=ld.p*i),rep("cumulative success or futility",times=ld.p*i),rep("sample size",times=ld.p*i))

        delta.control <- rep(delta.p,times=i*7)
        delta.treatment <- rep(delta.t,times=i*7)
        delta <- delta.treatment-delta.control

        stage <- rep(as.vector(aperm(array(data=paste("stage",1:i),dim=c(i,ld.t)),c(2,1))),times=7)
...
gsb <- function(design=NULL,
  ...
  if(class(simulation)[2]=="gsbSimArm"){
    ...
    if(class(r$warnings)[2]=="tr"){
      message(simulation$nr.sim," simlations per (delta.control,delta.treatment) is smaller than the specified threshhold = ",simulation$warnings.sensitivity ," for the following delta.control and delta.treatment. Further information in help of \"gsb()\": \n" )
  ...
  if(class(simulation)[2]=="gsbSimDif"){
    ...
    if(class(r$warnings)[2]=="tr")
      message(simulation$nr.sim," simlations per (delta.control,delta.treatment) is smaller than the specified threshhold = ",simulation$warnings.sensitivity ," for the following delta. Further information in help of \"gsb()\": \n" )
    w <- as.matrix(r$warnings)
    class(w) <- "matrix"
    print(w,digits=3)
  ...
  if(class(simulation)[2]=="gsbSimCalcDif"){
    ...
    if(class(r1$warnings)[2]=="tr"){
      cat(simulation$nr.sim," simlations per (delta.control,delta.treatment) is smaller than the specified threshhold = ",simulation$warnings.sensitivity ," for the following delta. Further information in help of \"gsb()\": \n" )
                      ...
                      prior.treatment="non-informative")
{
  ...
  if(class(sigma)=="matrix"){
    ...
    dimnames(sigma)[[1]] <- paste("stage",1:nr.stages)
    dimnames(sigma)[[2]] <- c("control", "treatment")
  }
